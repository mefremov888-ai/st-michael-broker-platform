    ...
    try {
      ...
      await this.prisma.$executeRaw`
        ...
        SET "comment" = regexp_replace(
          "comment",
          -- 2026-09-05: убран префикс E — E-строка съедала обратные слеши,
          -- \d превращался в диапазон "}-d" внутри [] и падал с 2201B
          -- каждые 10 минут с 01.07. Обычная строка передаёт \d как есть.
            ...
            if (!client) {
              ...
              client = await this.prisma.client.create({
                data: {
                  ...
                  // 2026-09-07 (правило владельца): дата фиксации = дата создания лида, не дата синка.
