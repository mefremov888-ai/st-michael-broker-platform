    ...
    if (isActivation && existingByPhone) {
      ...
      broker = await this.prisma.broker.update({
        ...
        data: {
          ...
          // показываем, просто привязываем карточку к нему и обновляем ФИО в
          // «Нашей базе». Самоназвание брокера приоритетнее имени, которое
          // за него записали КЦ или коллега в форме фиксации.
          displayName: data.fullName,
          displayNameSource: "self",
        },
        select: { id: true },
      });
    } else {
